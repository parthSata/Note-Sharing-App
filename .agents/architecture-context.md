...
The frontend is now wired to backend APIs through `frontend/src/lib/api.ts` and domain hooks:

- Auth uses backend login/register/logout responses and stores the JWT in `localStorage`.
- Note creation and listing use backend note endpoints.
- Share links and access keys are generated by the backend and shown once after creation.
- Note detail and public share routes are driven by API responses and backend error codes.
